...
log_info "Deploying BLOOM tenant resources for project: $PROJECT, environment: $ENV"

# Load common and tenant parameters
COMMON_CONFIG="${SCRIPT_DIR}/../../config/parameters-${ENV}.json"
TENANT_CONFIG="${SCRIPT_DIR}/../../config/bloom/parameters-${ENV}.json"
...
jq --arg tenant "$tenantName" --arg subnetName "$caeSubnetName" --arg subnetId "$subnetId" --arg subnetPrefix "$caeSubnetPrefix" --arg hostName "$domainName" \
   ...
   "$infra_file" > tmp.$$.json && mv tmp.$$.json "$infra_file"# 2. Create User-Assigned Managed Identity
...
jq --arg tenant "$tenantName" --arg psqlName "$psqlName" --arg psqlId "$psqlId" \
   ...
   "$infra_file" > tmp.$$.json && mv tmp.$$.json "$infra_file"# 4. Deploy Container App Environment
...
if [ -z "$caeId" ]; then
    ...
    az containerapp env update \
        ...
        --resource-group "$rgName" \
        --mi-user-assigned "$uamiId" \
        --output none 2>/dev/null || log_info "UAMI assignment complete"
fi
...
jq --arg tenant "$tenantName" --arg caeName "$caeName" --arg caeId "$caeId" \
   ...
   "$infra_file" > tmp.$$.json && mv tmp.$$.json "$infra_file"# 5. Deploy Container App
...
jq --arg tenant "$tenantName" --arg caName "$caName" --arg caId "$caId" --arg caIp "$caIp" \
   ...
   "$infra_file" > tmp.$$.json && mv tmp.$$.json "$infra_file"log_info "Tenant resources deployment for '${tenantName}' complete."
