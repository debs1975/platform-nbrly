...
# Build and Push Script for NBRLY Tenant Applications
...
build_and_push_app() {
    ...
    local app_path="../$TENANT/$app"
    
    log "Building and pushing NBRLY application: $app"
...
# Main execution
main() {
    log "Starting build and push process for NBRLY tenant applications"
    ...
    # Get NBRLY applications from configuration
    ...
    for app in "${nbrly_apps[@]}"; do
        log "Processing NBRLY application: $app"
        
        if build_and_push_app "$app"; then
            ((success_count++))
        else
            warning "Failed to build/push NBRLY application: $app"
        fi
    ...
    # Summary
    log "NBRLY build and push summary:"
    log "Successful: $success_count/$total_count"
    
    if [ $success_count -eq $total_count ]; then
        success "All NBRLY applications built and pushed successfully!"
        
        log "NBRLY images available in ACR:"
        for app in "${nbrly_apps[@]}"; do
            echo "  - $ACR_REGISTRY/$TENANT-$app:$TAG"
        done
        
        log "To deploy NBRLY applications, use: ./deploy-nbrly.sh"
...
# Help function
show_help() {
    echo "Build and Push Script for NBRLY Tenant Applications"
    echo
    echo "Usage: $0 [TAG]"
    echo
    echo "Arguments:"
    ...
    echo "  $0 nbrly-dev    # Build with 'nbrly-dev' tag"
    echo
    echo "Applications built:"
    echo "  - nbapp1 (nbrly-nbapp1:TAG)"
    echo "  - nbapp2 (nbrly-nbapp2:TAG)"
    echo
    echo "Prerequisites:"
