...
#
# EXAMPLES:
#   ./09-deploy-yaml-bloom.sh             # Deploy with 'latest' tag
#   ./09-deploy-yaml-bloom.sh v1.0.0      # Deploy specific version
#
# PREREQUISITES:
...
main() {
    ...
    declare -a manifests=(
        "${MANIFESTS_DIR}/bloom-bmapp1.yaml"
        "${MANIFESTS_DIR}/bloom-bmapp2.yaml"
    )
    ...
    for manifest in "${manifests[@]}"; do
        ...
        # Extract app key from filename (e.g., bloom-bmapp1.yaml -> bmapp1)
    ...
    if [ $success_count -eq $total_count ]; then
        ...
        error "Some BLOOM Container Apps failed to deploy"
        exit 1
    fi
}
