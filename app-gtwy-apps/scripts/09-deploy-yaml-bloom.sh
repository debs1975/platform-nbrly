...
# Deploy BLOOM Container Apps using YAML Manifests
#
# USAGE:
#   ./09-deploy-yaml-bloom.sh [ENV] [TAG]
#
# PARAMETERS:
...
#
# EXAMPLES:
#   ./09-deploy-yaml-bloom.sh                  # Deploy dev env with 'latest' tag
#   ./09-deploy-yaml-bloom.sh dev v1.0.0       # Deploy dev env with specific version
#   ./09-deploy-yaml-bloom.sh stage v2.0.0     # Deploy stage env with specific version
#
# PREREQUISITES:
#   - Azure CLI logged in (az login)
#   - Container Apps extension installed
#   - BLOOM YAML manifests generated in manifests/.generated/
#   - BLOOM images available in ACR with specified TAG
#
# WORKFLOW:
#   1. Generates manifests from templates
#   2. Updates UAMI client IDs and image tags
#   3. Deploys bloom-bmapp1 and bloom-bmapp2
...
main() {
    echo "================================================================================"
    log "DEPLOY BLOOM APPS USING YAML MANIFESTS"
    echo "================================================================================"
    log "Script: 09-deploy-yaml-bloom.sh"
    log "Purpose: Deploy BLOOM tenant apps using declarative YAML manifests"
    echo "-------------------------------------------------------------------------------"
    log "Parameters:"
    log "  Environment:         $ENV"
    log "  Tenant:              $TENANT"
    ...
    # Define BLOOM manifest files
    declare -a manifests=(
        "${MANIFESTS_DIR}/bloom-bmapp1-${ENV}.yaml"
        "${MANIFESTS_DIR}/bloom-bmapp2-${ENV}.yaml"
    )
    ...
    echo "================================================================================"
    log "STEP 4: DEPLOY BLOOM CONTAINER APPS"
    echo "================================================================================"
    echo "================================================================================"
    ...
    for manifest in "${manifests[@]}"; do
        ...
        # Extract app key from filename (e.g., bloom-bmapp1-dev.yaml -> bmapp1)
        local filename=$(basename "$manifest" .yaml)
        # Remove tenant prefix and environment suffix
        local app_key="${filename#${TENANT}-}"  # removes bloom-
    ...
    echo "================================================================================"
    log "YAML deployment summary for BLOOM:"
    log "Successful: $success_count/$total_count"
    
    if [ $success_count -eq $total_count ]; then
        success "All BLOOM Container Apps deployed successfully!"
        ...
        error "Failed to deploy some BLOOM Container Apps"
...
# Help function
show_help() {
    echo "Deploy BLOOM Container Apps using YAML Manifests"
    echo
    echo "Usage: $0 [TAG]"
    echo
    echo "Arguments:"
