...
update_manifest_tag() {
    ...
    # Get UAMI client ID from configuration
    local uami_client_id=""
    if [ -n "$tenant" ]; then
        uami_client_id=$(get_tenant_value "$tenant" "$ENV" ".managedIdentity.clientId")
...
show_help() {
    ...
    echo "  - Images available in ACR: astradevacr.azurecr.io"
    echo "  - Container App Environment: cae-astrapia-dev"
