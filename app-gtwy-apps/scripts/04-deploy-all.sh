...
# Deploy All Applications to Azure Container Apps (Orchestrator)
# Deploys all 4 applications (nbrly and bloom tenants) with proper configuration
# Calls 05-deploy-nbrly.sh and 06-deploy-bloom.sh
#
# USAGE:
#   ./04-deploy-all.sh [ENV] [TAG]
#
# PARAMETERS:
...
# EXAMPLES:
#   ./04-deploy-all.sh
#   ./04-deploy-all.sh dev v1.0.0
#   ./04-deploy-all.sh prod v2.1.0
#
# PREREQUISITES:
#   - Azure CLI logged in (az login)
#   - Container images already built and pushed to ACR
#   - config/infra-<ENV>.json configured
#   - Azure Container Apps environment exists
#   - Scripts 05-deploy-nbrly.sh and 06-deploy-bloom.sh available
#
# DEPLOYS:
#   - Calls 05-deploy-nbrly.sh to deploy nbapp1 and nbapp2
#   - Calls 06-deploy-bloom.sh to deploy bmapp1 and bmapp2
#   - All 4 applications deployed to Azure Container Apps
...
LOCATION=$(jq -r '.location' "$INFRA_CONFIG")
ACR_NAME=$(jq -r '.containerRegistry.name' "$INFRA_CONFIG")
ACR_REGISTRY="${ACR_NAME}.azurecr.io"
# Container app environments are tenant-specific, will be resolved per app
LOG_ANALYTICS_WORKSPACE=$(jq -r '.logAnalytics.name // "law-astrapia-dev"' "$INFRA_CONFIG")
...
show_help() {
    ...
    echo
    echo "Examples:"
    echo "  $0              # Deploy with 'dev' env and 'latest' tag"
    echo "  $0 dev v1.0.0   # Deploy with 'dev' env and 'v1.0.0' tag"
    echo "  $0 prod v2.0.0  # Deploy with 'prod' env and 'v2.0.0' tag"
    echo
    ...
    echo "  - Container App Environments: tenant-specific (auto-resolved)"
    echo
}
