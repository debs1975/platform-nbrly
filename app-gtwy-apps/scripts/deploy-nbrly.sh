...
LOCATION=$(get_global_value "$ENV" ".region")
ACR_NAME=$(get_global_value "$ENV" ".containerRegistry")
ACR_REGISTRY="${ACR_NAME}.azurecr.io"
CONTAINER_APP_ENV=$(get_tenant_value "$TENANT" "$ENV" ".containerAppEnvironment")
...
deploy_nbrly_app() {
    ...
    local uami_resource_id=$(get_tenant_value "$TENANT" "$ENV" ".managedIdentity.resourceId")
    ...
    if az containerapp show --name "$app_name" --resource-group "$RESOURCE_GROUP" >/dev/null 2>&1; then
        ...
        if [ "$app_key" == "nbapp2" ] || [ "$app_key" == "bmapp2" ]; then
            ...
            local kv_name=$(get_global_value "$ENV" ".keyVault.name")
            local db_secret=$(get_tenant_value "$TENANT" "$ENV" ".database.connectionStringSecret")
...
# Main execution
main() {
    log "Starting deployment process for NBRLY tenant applications"
