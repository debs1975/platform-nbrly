...
LOCATION=$(jq -r '.resources.resourceGroup.location // "East US"' "$INFRA_CONFIG")
ACR_NAME=$(jq -r '.resources.containerRegistry.name' "$INFRA_CONFIG")
ACR_REGISTRY="${ACR_NAME}.azurecr.io"
# Container app environments are tenant-specific, will be resolved per app
LOG_ANALYTICS_WORKSPACE=$(jq -r '.resources.logAnalytics.name // "law-astrapia-dev"' "$INFRA_CONFIG")
...
deploy_container_app() {
    ...
    local tenant_cae=$(jq -r ".resources.tenants.${tenant}.containerAppEnv.name" "$INFRA_CONFIG")
