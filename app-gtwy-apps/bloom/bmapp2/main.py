"""
BLOOM Sample FastAPI Application 1 for Azure Container Apps with Application Gateway
Demonstrates:
...
SAMPLE_API_KEY = os.getenv("SAMPLE_API_KEY", None)  # From Key Vault: bloom-api-key
DATABASE_URL = os.getenv("DATABASE_URL", None)  # From Key Vault: bloom-psql-connection-string
    ...
    return {
        ...
        "secrets": {
            ...
            "database_url": {
                ...
                "source": "KeyVault: bloom-psql-connection-string",
...
async def test_database() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "message": "Database URL is configured for bloom tenant",
            "tenant": "bloom",
            "app": "bmapp2",
            "timestamp": datetime.utcnow().isoformat()
        }
